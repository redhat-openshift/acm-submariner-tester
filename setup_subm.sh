...
(
  ...
  if [[ ! "$skip_tests" =~ ((sys|all)(,|$))+ ]]; then
    ...
    if [[ "$globalnet" =~ ^(y|yes)$ ]] ; then
      ...
      ${junit_cmd} test_globalnet_status_cluster_c
    fi
    ...
    ${junit_cmd} test_lighthouse_status_cluster_c
