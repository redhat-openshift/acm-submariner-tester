...
function export_LATEST_IIB() {
  ...
  LATEST_IIB="$(cat latest_iib.txt | jq -r '[.raw_messages[].msg | {nvr: .artifact.nvr, index_image: .pipeline.index_image}] | .[0]')"

  TITLE "Image indexes for bundle '${bundle_name}' \n $LATEST_IIB"
  ...
  LATEST_IIB=$(echo "$LATEST_IIB" | jq -r '.index_image."v'"${ocp_version_x_y}"'"' )
